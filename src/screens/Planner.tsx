...
export default function Planner({
  ...
  const originalBedtimeMinutes = parseTimeToMinutes(bedtimeStr);
  ...
  if (latestClassEndMinutes !== null) {
    let diff =
      shiftTime(originalBedtimeMinutes) - shiftTime(latestClassEndMinutes);
    sleepBufferMinutes = diff;
  }

  let hardStopMinutes = originalBedtimeMinutes - 90;
  let stopReasonText = "90m before bedtime";

  if (latestClassEndMinutes !== null && sleepBufferMinutes < 90) {
    hardStopMinutes = latestClassEndMinutes;
    stopReasonText = "After last class";
  }

  const activeStopTime = formatTime(formatMinutes(hardStopMinutes));

  return (
    <div
      className="flex flex-col min-h-screen pb-24 relative"
    ...
    >
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="relative">
              <button onClick={onBellClick} className="text-slate-100">
                <Bell size={24} />
              </button>
        ...
        <div className="flex gap-3 overflow-x-auto pb-2 -mx-4 px-4 no-scrollbar">
          {["Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((day, i) => {
            ...
            return (
              ...
              >
                <p
                  className={`text-xs uppercase font-semibold ${isActive ? "opacity-80" : "text-slate-400"}`}
                >
                  {day}
                </p>
      ...
      <main className="flex-1 px-4 py-4 space-y-8">
          ...
          <div className="relative ml-4 border-l-2 border-slate-800">
            ...
            <div className="relative pl-8 pb-0 mt-4">
              ...
              <div className="flex flex-col mb-4">
                ...
                <p className="text-xs text-red-400/80">{stopReasonText}</p>
              </div>
            </div>
              ...
              <div className="flex flex-col">
                <div className="flex items-center gap-2 text-primary mb-1">
                  ...
                  <span className="text-xs font-bold tracking-widest uppercase">
                    Automatic Sleep Buffer
                  </span>
                </div>
                <p className="text-sm text-slate-300">
                  Optimizing cognitive cooldown until bedtime...
                </p>
              </div>
            </div>
        ...
        <section className="mb-8">
          <button
            ...
            className="w-full p-4 rounded-xl flex items-center justify-between bg-white/5 border border-white/10 backdrop-blur-md active:scale-[0.98] transition-all group"
          >
            <div className="flex items-center gap-4">
          ...
          <div className="space-y-3">
            {filteredTasks.length === 0 ? (
              ...
              filteredTasks.map((task) => (
                <div
                  key={task.id}
                  className={`p-4 rounded-xl flex items-center gap-4 bg-white/5 border border-white/10 backdrop-blur-md transition-all ${task.completed ? "opacity-50" : ""}`}
                >
                  <button
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      <h4
                        className={`text-sm font-bold ${task.completed ? "line-through text-slate-400" : ""}`}
                      >
                        {task.title}
                      </h4>
                      {task.category && (
      ...
      {isViewAllOpen && (
        <div
          className="fixed inset-0 z-50 flex items-end justify-center bg-background-dark/80 backdrop-blur-sm"
        ...
        >
          ...
          >
            <div className="flex items-center justify-between mb-6">
              ...
              <button
                onClick={() => setIsViewAllOpen(false)}
                className="p-2 rounded-full bg-white/5 text-slate-400 hover:text-slate-100"
              >
                <X size={20} />
              </button>
            </div>
            ...
            >
              {tasks.length === 0 ? (
                ...
                tasks.map((task) => (
                  <div
                    key={task.id}
                    className={`p-4 rounded-xl flex items-center gap-4 bg-white/5 border border-white/10 transition-all ${task.completed ? "opacity-50" : ""}`}
                  >
                    <button
                      onClick={() => toggleTaskCompletion(task.id)}
                      className={`w-6 h-6 rounded-lg border-2 flex items-center justify-center transition-colors ${task.completed ? "bg-primary border-primary text-background-dark" : "border-primary/30 text-transparent"}`}
